...
5. **`portableTextBlocks`のブロックには`fields`を1つ以上持たせる。** 管理エディタは
   `blockDef.fields.length > 0`で分岐しており、`fields`が無い(`[]`も同じ)ブロックは挿入時にIDかURLの
   入力を求められる。さらに、`id` / `url` / フィールド値のどれも持たないブロックはエディタへ読み込むときに
   `[Unknown block type: <type>]`という段落に置き換わり、そのまま保存すると**本文のテキストとして
   永続化される**。表示データを自前で引く「マーカー」的なブロックでも、`toggle`(例: 表示/非表示)などの
   フィールドを1つ置く。宣言の変更なのでDBの移行は要らず、サイト側の描画はもともと影響を受けない。
