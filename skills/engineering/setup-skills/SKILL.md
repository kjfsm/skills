---
name: setup-skills
description: このリポジトリをエンジニアリング系スキル向けに設定する — イシュートラッカー、トリアージラベルの語彙、ドメインドキュメントの配置、応答と記述の規約をセットアップする。他のエンジニアリング系スキルを初めて使う前に一度実行する。
...
編集したのが `CLAUDE.md` だった、あるいは `.claude/` が存在するなら、続けて `/setup-rules` を実行する。ここが書いたのは毎セッション読まれる層だけで、パスに応じて注入される層(`.claude/rules/`)はそちらが敷く。`AGENTS.md` しか無いリポジトリでは飛ばす — `.claude/rules/` を読まないハーネスが対象なので、置いても届かない。
