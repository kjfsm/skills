...
スキル本文がほかのスキルを名指しするとき、**誰が動くのか**で表記を分ける。

- **モデルが呼ぶ** → `` `grilling` スキルを呼ぶ ``。スラッシュを付けない
- **ユーザーがタイプする** → `` `/kjfsm-skills:setup-skills` を実行してもらう ``。スラッシュとプラグイン名を付ける — プラグインのスキルは `/<プラグイン>:<名前>` で打つ(本家なら `/mattpocock-skills:<名前>`)

本文の中の `/x` は「人間がタイプするもの」として読まれるので、モデルへの指示に使うと呼ばずに読み流される。本家 mattpocock/skills はこれを「Skill ツールを "x" で呼ぶ」と書くが、こちらは **道具名を出さない** — `npx skills` で Codex にも配っており、`Skill` ツールという名前は Claude Code 固有だからである。

`ask-kjfsm` と `setup-repo` の工程表はユーザーに読ませるルーターなので、`/x` のままでよい。`/context`、`/init`、`/doctor` はハーネス組み込みのコマンドで、これもユーザーがタイプする側である。
