...
**非対話モードが無いのは、機能の欠如ではなくバージョンの問題である。** 公式が非対話を持つ系列が stable で出ているなら、**上げることを検討する** — 上がれば手順2以降は要らなくなる。材料はレジストリに訊く:

```
npm view drizzle-kit dist-tags
```

`latest` が非対話モードを持つ系列に乗っていれば、検討に値する。prerelease(`beta` / `rc`)にしか無いなら、それを本番の DB に向けるかどうかという別の判断になる。

**上げるかどうかは、このスキルが決めることではない。** drizzle-kit だけを上げることはできず、drizzle-orm と組でしか動かない(古い orm に新しい kit を載せると `ERR_PACKAGE_PATH_NOT_EXPORTED` で起動しない、実測)。アプリのランタイムと生成物の配置(→ D1 / wrangler と組み合わせるとき)を巻き込むので、**上げるならユーザーに諮ってから決める。** 見送るなら 2 へ。
