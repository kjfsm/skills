...
**`kjfsm-skills` はこれに依存しない。** 依存先は本家の英語版 `mattpocock-skills@mattpocock` で、`kjfsm-skills` を入れると一緒に入る。このバケットは本家と同じ名前のスキルを持つので、本家と併用すると同じ名前が2度並ぶ — 日本語で読みたい人が本家の代わりに入れるものである。**`kjfsm-skills` を入れると本家が依存として一緒に入るので、`kjfsm-skills` の利用者はこれを併用できない。**理由は [ADR 0006](../../.agents/adr/0006-depend-on-upstream-ship-translation-separately.md)。
...
訳は自作スキルへの参照を持たない — 片方だけ入れても壊れないように。docs/agents/ の設定は本家の `/setup-matt-pocock-skills`(英語)を案内する。その訳はこのバケットに無いので、本家のプラグインから打つ。
