...
**`generate` はスキーマから DDL を再生成するだけである。履歴に混ざったデータ文は再生成されない。**

```
grep -liE '^\s*(UPDATE|INSERT|DELETE)\b' <migrations_dir>/*.sql
```

1件でも出たら、**それをどう運ぶか決めるまで先へ進まない。** 選択肢は3つ。

- **捨てる** — そのデータが本番にしか無く、新しい環境では要らないなら。ただし fresh な local / preview は以後そのデータを持たなくなる
- **squash の範囲から外す** — データ文を含むファイルより手前だけを畳む
- **書き直して残す** — 畳んだ init の後ろに、独立したマイグレーションとして置き直す。**元のファイルをそのまま持ち越さない** — backfill は当時のスキーマに対して書かれており、畳んだ後の最終スキーマに対しては動かないことがある

**完了基準: 該当ファイルの一覧と、それぞれをどう扱うかを言えること。**
