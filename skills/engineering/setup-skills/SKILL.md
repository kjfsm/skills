...
このセクションが書くものだけは、他と違って **ブロックの中に本文を置く**。`docs/agents/*.md` は必要になったときに読まれるが、規約は毎セッション読まれなければ意味がない。コメントの判定基準をスキルへの参照で済ませないのも同じ — スキルは呼ばれて初めて読まれるのに、コメントを書く場面でモデルは「迷った」と自覚せず呼ばない。それでも 8 行に収める — 書き込む先は、長いほど従われなくなる。
