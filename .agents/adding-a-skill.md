...
`agents/<名前>.md` に置く。`plugin.json` への登録は要らない — プラグインはこのディレクトリを自動で走査する。**列挙しないことが正しい状態である**(→ `CLAUDE.md`)。
...
書いたら `scripts/check-invariants.sh` — 名前とファイル名の一致、`description` と `model` の有無、**どのスキルからも名指しされていないエージェント** を検査 18. が弾く。
