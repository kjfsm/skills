...
- 本家から離れて自作の流れの中心になった `implement`・`ask-kjfsm`・`setup-skills`・`writing-great-skills` は `kjfsm-skills` に残す。
...
- 本家と同じ名前のスキルは、`kjfsm-skills` には置かない。例外は `implement` だけで、ユーザーの判断で名前を据え置いた。`/implement` を打つと本家版と kjfsm 版の2つが並び、本家の流れ(`ask-matt` や `to-tickets`)から呼ばれたときにどちらが選ばれるかはモデル次第になる。kjfsm 版が選ばれなければ、検証 → コメント削り → 二軸レビューの流れは黙って飛ばされる。
...
- `implement` の衝突で、実際に流れが飛ばされたら、kjfsm 版を改名する。
