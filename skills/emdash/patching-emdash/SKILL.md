...
- コミットメッセージに **なぜ当てたか**(観測した症状と数字)、**上流の状況**(main でも同じか、issue / PR の番号)、**いつ外せるか** を書く。パッチの理由を書く docs は作らない — 上げるときに読むのはコミットログである。
- 上流への issue / PR は外部への投稿なので、出す前にユーザーに確かめる。既存の近い issue(同じ症状の読み取り版など)があれば番号を添える。

## EmDash を上げるとき

版付きキーは新しい版に当たらないので、install の前にパッチを1本ずつ判定する。

1. **上流で直ったか。** 新しい版の該当ファイルを読む。直っていれば、パッチファイルと `patchedDependencies` のキーを消す。
