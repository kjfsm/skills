...
PAT発行が目的なら「手順」節だけ読めばよい。プラグイン設定・D1の直接参照・MCPに無い操作は、
末尾の「ここから先は分岐する」から別ファイルへ進む。
...
loopback URL)を指すエントリを探す——慣例として `-local` サフィックスが付く。
...
エントリの形は2通りあり、**`headers` を持つかどうかで復旧手順が変わる**。まず自分の
`.mcp.json` がどちらなのかを確認すること。

### (a) `headers` を持たない場合(既定)
...
この形のときは接続はOAuthで行われ、**`EMDASH_LOCAL_PAT` は誰も読まない**——この変数を
設定しても何も起きないので、401の復旧手段として `export` しないこと。`headers.Authorization`
を静的に置くとOAuthフォールバックが無効化されるため、PATを使わない構成では意図的に
ヘッダーを書かない。ローカルdevサーバーに対するOAuthはブラウザでの承認が要るので、
ブラウザを使えない状況では下記の手動curl手順(ステップ1〜3)へ進むこと。

### (b) `Authorization` ヘッダーでPATアクセスを使う場合
...
**この(b)の構成に限り**、コネクタが見つからない・未認証・呼び出しが401になる原因は
「環境変数がまだ設定されていない」か「それが指すPATが失効している」(例: `.wrangler/` が
削除された——下記の注記を参照)のどちらかである。対処法:
...
> **ヘッダーを静的に置くことの副作用**: `headers.Authorization` があるとOAuthフォールバックが
> 無効化される。そのため環境変数が未設定の環境(別マシン、CI、リモートのセッションなど)では
> `Bearer ${EMDASH_LOCAL_PAT}` が未展開の文字列のまま送信され、`401 INVALID_TOKEN` になる。
> 「トークンが失効した」ように見えて実際は「変数が無い」だけ、という誤診をしやすい。
> どの環境からでも繋がるようにしたいなら、(a)のヘッダー無しにしてOAuthに寄せる。

どちらの構成でも、現在のコンテキストで再接続ができない場合は下記のステップ3に
フォールバックできる(`Authorization` ヘッダーにトークンを直接載せた素のcurl)——
コネクタの登録方法に関係なく動作する。
