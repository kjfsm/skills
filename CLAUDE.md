...
その3か所は常に読まれる。それでも守られないので、**読まれることと、いま自分がその場面にいると気づくことは別である** — 4か所目を同じ「常に読まれる」層(`SessionStart` フックの標準出力も同じ場所に入る)へ足しても、同期先が1つ増えるだけで何も変わらない。埋めるのはプラグインが出荷する `hooks/hooks.json` の `PostToolUse` フックで、`Edit`/`Write` でコメント行が**増えたときだけ**鳴り、増えた行を1行ずつ点検させる。フックにしかない梃子は注入ではなく**発火の条件性**である。

このフックは常駐する規約の4か所目ではない。4本の柱を復唱せず(検査 17. が見る)、コメントが適切かも判定しない — 既にコンテキストにある基準を当てる作業をモデルへ差し戻すだけである。判定まで寄せると `setup-hooks` の条件 3 に触れ、通ってよい編集を止めた瞬間にモデルは `Bash` の `sed` へ逃げる。その `Bash` 経由の書き込みは実際に素通しする: `FileChanged` は監視対象をファイル名で literal に列挙する必要があり任意のソースファイルには使えないので、承知のうえで `Edit|Write` に掛けている。**捕まえられるのは増えたコメントだけで、書かれなかった Why not は捕まえられない** — そちらは `/two-axis-review` の受け持ちである。フックはサブエージェントの中でも走るので、出力スタイルが届かない側の穴も同時に埋まる。
