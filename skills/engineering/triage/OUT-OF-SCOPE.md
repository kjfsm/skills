...
- [ディレクトリ構造](#ディレクトリ構造)
- [ファイル形式](#ファイル形式)
- [いつ `.out-of-scope/` を確認するか](#いつ-out-of-scope-を確認するか)
- [いつ `.out-of-scope/` に書くか](#いつ-out-of-scope-に書くか)
- [スコープ外ファイルの更新・削除](#スコープ外ファイルの更新・削除)
...
**拡張**(バグではなく)が `wontfix` として _却下_ されたときだけ。これは拡張系の PR にも、イシューとまったく同じように当てはまる — 却下された PR はここに記録され、同じ要求が新しいコードとして戻ってこないようにする。
