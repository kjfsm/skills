...
- **[build-browser-game](./build-browser-game/SKILL.md)** — スマホでも遊べるブラウザゲームを、kjfsm の構成(Workers・Durable Objects・kjfsm-auth・共有 D1)で新しく作って本番に出すまでの手順。「ゲームを作って」と頼まれたとき、リアルタイム対戦を Durable Objects で組むとき、ゲームの数(強さ・勝負の長さ)を調整するとき、対戦で幽霊の人・遅れて鳴る演出・誤った引き分けが出たときに使う。
