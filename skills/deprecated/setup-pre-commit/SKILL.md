...
pnpm なら、入れる前に `pnpm up --latest` で既存の依存を最新にする(ワークスペースなら `-r`)。範囲を無視してメジャーまで上がるので、typecheck と test が通るのを確かめて別コミットにしてから次へ進む — 手順8のコミットに混ぜると、赤くなったときに上げた依存とフックの切り分けがつかない。
