...
上から順に当たり、どれかで解決するならパッチを書かない。その前に [KNOWN-PATCHES.md](KNOWN-PATCHES.md) を読む — 同じ症状をすでに別のサイトで直していれば、その節の直し方と上流の追跡を引き継ぐ。
...
- 直した行の直前に、`/** [patched: <出どころ>] <なぜ> */` の1行コメントを置く(既存パッチの慣習)。`dist/` は英語のビルド成果物なので英語で書く。
...
**完了基準:** `pnpm install` が通り、`grep -n "\[patched:" node_modules/<pkg>/dist/<file>` が当たる。`pnpm-lock.yaml` の `patchedDependencies` にハッシュが増えている。
...
KNOWN-PATCHES.md の正本は kjfsm/skills にある。インストール済みのプラグインの中の写しを直しても、次の更新で上書きされて消える — kjfsm/skills のチェックアウトで直し、サイトとは別の PR にする。チェックアウトも書き込み権も無ければ、足す内容をユーザーに渡して止まる。

- **このスキルの [KNOWN-PATCHES.md](KNOWN-PATCHES.md) に節を足す。** 節は「当てる理由」(症状、観測した数字、当てる条件と要らない条件)と「当てる内容」(直すファイル、変える行、その書き方を選んだ理由)を中心に、当て始めた日・上流の状況・外せる条件を添える。サイト名やリポジトリ名は書かない — どのサイトで当てているかはサイト側の `patchedDependencies` が持つ。サイトのコミットログに散らばると、別のサイトで同じ症状に当たったときも、上げるときの見直しでも拾えない。
- サイトのコミットメッセージには、そのサイトで当てた理由を書く。
- 上流への issue / PR は外部への投稿なので、出す前にユーザーに確かめる。既存の近い issue(同じ症状の読み取り版など)があれば番号を添える。出したら KNOWN-PATCHES.md の「上流」を更新する。

**完了基準:** KNOWN-PATCHES.md にそのパッチの節があり(または節の本文をユーザーに渡した)、「外せる条件」が上流のコードで判定できる形で書けている。

## EmDash を上げるとき

版付きキーは新しい版に当たらないので、install の前に、サイトの `patchedDependencies` にあるパッチを1本ずつ判定する。判定の材料(直す場所・外せる条件)は [KNOWN-PATCHES.md](KNOWN-PATCHES.md) にある。

1. **上流で直ったか。** KNOWN-PATCHES.md の「外せる条件」を、新しい版の該当ファイルで確かめる。直っていれば、パッチファイルと `patchedDependencies` のキーを消す。
2. **直っていなければ作り直す。** 手順 4 を新しい版の `dist/` に対して行い、ファイル名とキーの版を上げる。**中身が同じでも行番号とチャンク名が動くので、古いパッチの版だけ書き換えても当たらない**(`ERR_PNPM_PATCH_FAILED`)。
3. コミットメッセージに、パッチごとに「直っていなかったので作り直した / 直ったので外した」を1行ずつ書く。
4. KNOWN-PATCHES.md の「作り直しの履歴」に日付と版を足す。外したなら、その節に外した日と、直った上流の版を書く。書く先は手順 6 と同じく kjfsm/skills のチェックアウトである。

**完了基準:** `patchedDependencies` のすべてのキーが新しい版になっているか消えていて、KNOWN-PATCHES.md の履歴が追いついている。
