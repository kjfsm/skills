...
「いいえ」なら応答言語だけ差し替える。宛先の規律は差し替えない — あれはスキル側が持っており、ここはそれを指す 1 行でしかない。`where-to-write-what` がインストールされていなければ、その行を省いて「コードを読めば分かることは書かない」だけを残す。

このセクションが書くものだけは、他と違って **ブロックの中に本文を置く**。`docs/agents/*.md` は必要になったときに読まれるファイルだが、規約は毎セッション読まれなければ意味がないためである。8 行に収める — このファイルは行数が増えるほど従われなくなる。
...
コードを読めば分かることは書かない — 処理の逐語的な説明、変更の経緯、構成の概観。書いたものを消してコードだけを読み、失われる情報が無ければ、その宛先はコードだった。

コメント・JSDoc・コミットメッセージ・PR 本文・ADR・docs のどれに何を書くかは `where-to-write-what` スキルが決める。
```
