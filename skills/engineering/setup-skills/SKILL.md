...
「いいえ」なら応答言語だけ差し替える。宛先の規律は差し替えない。`where-to-write-what` がインストールされていない場合も、コメントの判定はこのブロックが単体で持っているので、スキルを指す最後の 1 段落だけを落とす。

このセクションが書くものだけは、他と違って **ブロックの中に本文を置く**。`docs/agents/*.md` は必要になったときに読まれるファイルだが、規約は毎セッション読まれなければ意味がないためである。コメントの判定基準をスキルへの参照で済ませないのも同じ理由 — スキルは呼ばれて初めて読まれるのに、コメントを書く場面でモデルは「迷った」と自覚せず呼ばない。それでも 8 行に収める — このファイルは行数が増えるほど従われなくなる。
...
ハーネスの出力スタイルで同じことをすでに指定している場合も、この行は消さない。**出力スタイルはサブエージェントに届かないが、このファイルは届く**(組み込みの探索用エージェントを除く)。レビューや調査を子コンテキストに投げたときに規約が効くのは、こちら側だけである。
