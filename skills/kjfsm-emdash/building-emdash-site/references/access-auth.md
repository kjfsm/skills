...
> ロールがどこから来るか**に触れていない。ここに書くのはそこだけ。根拠は EmDash 0.40.0 の
...
| `/_emdash/api/oauth/token`、`/_emdash/api/oauth/register`、`/_emdash/api/oauth/device/code`、`/_emdash/api/oauth/device/token` | MCP の OAuth のうち機械が叩く側                                                      |
| `/_emdash/.well-known/`                                                                                                        | MCP クライアントの OAuth ディスカバリ                                                |
| `/_emdash/api/snapshot`                                                                                                        | プレビューサービス。`X-Preview-Signature` の署名で認証する。プレビューを使うときだけ |
| `/_emdash/api/plugins/<id>/<route>`(`public: true` のもの)                                                                     | プラグインの公開ルート                                                               |

MCP の OAuth の同意画面(`/_emdash/oauth/authorize`)とデバイスフローの承認(`/_emdash/api/oauth/device/authorize`)はブラウザから人が叩くので、Access の内側に置いたままでよい。`/_emdash/api/oauth/device/` を前方一致で bypass すると、承認まで外に出る。ルート直下の `/.well-known/oauth-*` は `/_emdash` の外なので、Access を `/_emdash` に掛けている限り気にしなくてよい。
