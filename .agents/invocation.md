...
- **モデル呼び出し型** — **モデルからもユーザーからも**到達できる。デフォルトはこちら: `disable-model-invocation` と `agents/openai.yaml` の `policy` ブロックを省略する。あるスキルをモデル呼び出し型のままにすべきかどうかの判定基準は: _モデルがこれを自律的に使いこなせるか?_ である(再利用性はスキルを切り出す理由ではあっても、この判定基準ではない)。

  モデル呼び出し型の frontmatter は、2つの仕事を2つのフィールドに分ける: `description` はそのスキルが **何であるか** を1〜2文で述べ、`when_to_use` が **いつ発火すべきか** のトリガー表現("ユーザーが…したいとき、…に言及したとき、他のスキルが…を必要とするとき")を持つ。ハーネスは両者を連結してスキル一覧に載せる(合計1,536文字で打ち切られる)ので、トークンの節約にはならないが、人間が読む1行とモデルが照合する文字列が混ざらなくなる。
