...
| `p55` | [Prompting Claude Opus 5.5][p55]              | Opus 5.5 向けのプロンプト。effort、早すぎる停止、推論の再現の拒否、時間の信号(2026-09-22)        | 2026-09-25 |
...
Opus 5.5 は毎回の返信の前に必ず考え、考える量を自分で決める。「think carefully」「think step by step」はプロンプトからも保存した指示からも外す — チャット製品での試験で、外すと返信の出だしが速くなり、品質の明確な低下は無かった。考える量を変えたいなら文言ではなく **effort** を変える(文言より確実に効く)。**effort の名前はモデル間で同じ思考量を指さない** — Opus 5.5 の既定は `medium` で、`medium` が Opus 5 の `high` と同等以上、同じ段なら Opus 5 より多く考える。前のモデルの設定を持ち越さず、`xhigh` / `max` は効きを測ったときだけ使う。**内部の推論を返信に再現させる依頼は、`reasoning_extraction` として拒否されうる。** 理由が欲しいなら「このやり方を選んだ理由を3文で」のように結論として求める。 — [o5] / [p55]
...
Opus 5.5 は、名指しされた停止に従う。避けたい早すぎる停止と、望む停止の両方を名指しする。公式が名指ししている早すぎる停止は4類型: (1) 次の手順を告げる要約でターンを終え、その手順を始めない (2)「よければ続けます」と申し出て、来ない返事を待つ (3) 自分の説明でもどれも残りの作業を塞がない決定事項を並べて止まる (4) ターンが長くなった・区切りがついたという理由で報告に切り替える。状況の報告と推奨は、次のツール呼び出しと同じメッセージに入れさせる。望む停止は、自分なしでは何も進まないときと、塞いでいるものが意図的に保護されているときである。ツール呼び出しの無いメッセージはターンを終えるので、**無人で走るサブエージェントでは、これが途中で打ち切られた最終報告になる**。`CLAUDE.md` に置く例: 「入力が要らない手順は進み続ける。状況の報告は次の行動と同じメッセージに入れる。止まって訊くのは、自分なしでは進めないときと、破壊的な操作(データの削除、force push、このリポジトリの外の変更)の前だけ」。ペアプログラミングなら逆(着手前に1行の計画、最後に短い要約)を書く — どちらにも従う。危険な操作の権限プロンプトは残しておく。 — [o5] / [p55]

### タスク一覧はファイルに置く

「`TASKS.md` にチェックリストを置き、終えたら消し込み、見つけたものは足す」。古いターンは要約されるが、ファイルの一覧は残る。テキストだけでターンが終わったら、それは完了の証明ではなく報告として扱う — 開いた項目が残っていて障害も書かれていなければ、項目を名指しして続けさせる。自動の続行は同じタスクで2〜3回までにし、本当に詰まった実行は止めて人が見る。背景のコマンドやサブエージェントが走っている間も完了扱いにしない。 — [o5] / [p55]
...
PDF を画像に変換してレイアウトを視覚的に解析させるなど、視覚能力を使える形に変換する。 — [bp]。Opus 5.5 は道具なしでも密なチャートや図を前世代より正確に読むので、前のモデル向けに組んだ視覚入力の足場は要るかを測り直す。それでも最も密な入力(技術図面など)には、高解像度の画像と、切り抜き・拡大・計測ができる道具(PIL / OpenCV の入ったコンテナ、無ければ切り抜きの道具だけでも)が精度を足す。 — [p55]
...
- **時間の信号。** Opus 5.5 は経過時間に敏感で、親が子に委譲する構成では、時間の予算(「経過 340s / 1200s」を毎メッセージ末尾に)を渡すと並列を増やして早く終える。予算を決められないなら経過時間だけを見せ、「時間が重要: 避けられる時間は使わない。正しい結果は早いほど良い」を1文足す。effort を下げると仕事そのものが減るが、予算は主に並列の数を増やす。予算は助言で止めはしないので、ハードな停止はハーネス側のタイムアウトで持つ — [p55]
- **行動の前に探索させる。** 複数のコネクタ(メール・文書・表・CRM)をまたぐ作業では、Opus 5.5 はすぐ手を動かしがちで、依頼が名指ししていない場所(古いスレッドの方針、別タブの規則)を見落とす。「行動する前に、依頼が触れていないものも含めて関係しうるものを広く開き、見つけたものを使う」を1文足す。見つけたものに従わせる指示なので、信頼できない内容を探索範囲に置かない — [p55]
...
- [ ] 推論を返信に再現させる依頼が無い
- [ ] `effort` を前のモデルから持ち越していない。`xhigh` / `max` は効きを測ってある
- [ ] 無人で長く走るスキル・サブエージェントは、避けたい早すぎる停止と望む停止を名指ししている
- [ ] 強調は無視され続けた行だけに付いている
