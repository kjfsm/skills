...
- **新スキル `implement-spec`**(本家も in-progress) — worktree ごとの実装サブエージェントで最大限に並列化する形で、`implement` + `verification-loop` + `two-axis-review` の直列チェーンと正面衝突する。**「チケットは手順のリストではなくタスクグラフである」という捉え方だけを `implement` に1文で取り込んだ。** 並列実行そのものは、ゲートを薄めてまで欲しくなったときに別途決める
